...
def init_public_dashboard(server):
    public_app = Dash(
        ...
        url_base_pathname="/public_nba_dashboard/",
    )
    ...
    def update_win_loss_chart(new_df, new_rec_bet_df):
        """Create a bar chart of win/loss information based on the bet category.

        Args:
            new_df (pandas.DataFrame): A DataFrame containing data on the simulated bets.
    ...
    def update_profit_loss_chart(new_df, new_rec_bet_df):
        """
    ...
    def create_original_charts(df):
        ...
        simulated_rec_bet_win_count = rec_bet_df[
            rec_bet_df["rec_bet_win_loss"] == "Win"
        ]["rec_bet_win_loss"].count()
        simulated_rec_bet_loss_count = rec_bet_df[
            rec_bet_df["rec_bet_win_loss"] == "Loss"
        ]["rec_bet_win_loss"].count()
    ...
    public_app.layout = dbc.Container(
        children=[
            dbc.Row(
                [
                    dbc.Col(
    ...
    def update_output(start_date, end_date):
        ...
        new_profit_loss_chart = update_profit_loss_chart(new_df, new_rec_bet_df)

        # Bet Win Loss Bar Chart
        new_bet_win_loss_chart = update_win_loss_chart(new_df, new_rec_bet_df)
