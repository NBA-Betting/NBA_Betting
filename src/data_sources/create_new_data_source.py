...
def create_scrapy_spider(data_source_name, project_section, data_structure):
    ...
    # File contents
    imports_settings_class = f"""import json
import os
import sys
import re
...
from data_source_utils import BaseSpider, BaseSpiderZyte, find_season_information, convert_season_to_short
...
"""
    ...
    zyte_spider = f"""
class {class_name}Zyte(
    BaseSpiderZyte, {class_name}
):
    name = "{spider_name}_zyte"
    pipeline_name = "{to_camel_case(data_source_name)}Pipeline"
    project_section = "{to_snake_case(project_section)}"
    ...
    full_file_contents = (
        imports_settings_class + start_requests_method + parse_method + zyte_spider
    )
