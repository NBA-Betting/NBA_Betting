...
import pytz
import scrapy
from data_sources.item_loaders import NbaStatsPlayerGeneralUsageItemLoader
from data_sources.items import NbaStatsPlayerGeneralUsageItem
...
class NbaStatsPlayerGeneralUsageSpider(BaseSpider):
    name = "nba_stats_player_general_usage_spider"
    allowed_domains = ["www.nba.com"]
    custom_settings = {
        "ITEM_PIPELINES": {
            "data_sources.pipelines.NbaStatsPlayerGeneralUsagePipeline": 300
        },
    }
    ...
    def parse(self, response):
        ...
        for row in row_set:
            ...
            loader = NbaStatsPlayerGeneralUsageItemLoader(
                item=NbaStatsPlayerGeneralUsageItem()
            )
            ...
            loader.add_value("pct_fgm", row_dict["PCT_FGM"])
            loader.add_value("pct_fga", row_dict["PCT_FGA"])
            loader.add_value("pct_fg3m", row_dict["PCT_FG3M"])
            loader.add_value("pct_fg3a", row_dict["PCT_FG3A"])
            loader.add_value("pct_ftm", row_dict["PCT_FTM"])
            loader.add_value("pct_fta", row_dict["PCT_FTA"])
            loader.add_value("pct_oreb", row_dict["PCT_OREB"])
            loader.add_value("pct_dreb", row_dict["PCT_DREB"])
            loader.add_value("pct_reb", row_dict["PCT_REB"])
            loader.add_value("pct_ast", row_dict["PCT_AST"])
            loader.add_value("pct_tov", row_dict["PCT_TOV"])
            loader.add_value("pct_stl", row_dict["PCT_STL"])
            loader.add_value("pct_blk", row_dict["PCT_BLK"])
            loader.add_value("pct_blka", row_dict["PCT_BLKA"])
            loader.add_value("pct_pf", row_dict["PCT_PF"])
            loader.add_value("pct_pfd", row_dict["PCT_PFD"])
            loader.add_value("pct_pts", row_dict["PCT_PTS"])
