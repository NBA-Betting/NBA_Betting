...
import pytz
import scrapy
from data_sources.item_loaders import NbaStatsPlayerGeneralOpponentItemLoader
from data_sources.items import NbaStatsPlayerGeneralOpponentItem
...
class NbaStatsPlayerGeneralOpponentSpider(BaseSpider):
    name = "nba_stats_player_general_opponent_spider"
    allowed_domains = ["www.nba.com"]
    custom_settings = {
        "ITEM_PIPELINES": {
            "data_sources.pipelines.NbaStatsPlayerGeneralOpponentPipeline": 300
        },
    }
    ...
    def start_requests(self):
        base_url = "https://stats.nba.com/stats/leagueplayerondetails"
        headers = {
            "Accept": "*/*",
    ...
    def parse(self, response):
        ...
        for row in row_set:
            ...
            loader = NbaStatsPlayerGeneralOpponentItemLoader(
                item=NbaStatsPlayerGeneralOpponentItem()
            )
            loader.add_value("to_date", to_date)
            loader.add_value("season_year", season)
            loader.add_value("season_type", season_type)
            loader.add_value("vs_player_id", row_dict["VS_PLAYER_ID"])
            loader.add_value("vs_player_name", row_dict["VS_PLAYER_NAME"])
            ...
            loader.add_value("court_status", row_dict["COURT_STATUS"])
