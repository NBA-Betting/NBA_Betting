...
from items import NBA_FourFactorsStatsItem
from item_loaders import NBA_FourFactorsStatsItemLoader
from helpers import return_season_dates


class NBA_Four_Factors_Stats_Spider(Spider):
    name = "NBA_four_factors_stats_spider"
    ...
    # start_urls = [
    #     f"https://www.nba.com/stats/teams/four-factors/?sort=TEAM_NAME&dir=-1&Season=2020-21&SeasonType=Regular%20Season&DateTo={start_month}%2F{start_day}%2F{start_year}"
    # ]

    def __init__(self, season_dates=None, *args, **kwargs):
        super(NBA_Four_Factors_Stats_Spider, self).__init__(*args, **kwargs)
        self.season_dates = season_dates
        self.start_urls = [
            f"https://www.nba.com/stats/teams/four-factors/?sort=TEAM_NAME&dir=-1&Season={self.season_dates['season_years']}&SeasonType=Regular%20Season&DateTo={self.season_dates['final_month']}%2F{self.season_dates['final_day']}%2F{self.season_dates['final_year']}"
        ]
    ...
    def parse(self, response):
        ...
        for row in response.xpath("//div[@class='nba-stat-table']//tbody/tr")[
            ...
            loader = NBA_FourFactorsStatsItemLoader(
                item=NBA_FourFactorsStatsItem(), selector=row
            )
            loader.add_value("date", date_to)
            loader.add_xpath("team", "td[2]/a/text()")
            loader.add_xpath("fta_rate", "td[9]/text()")
            loader.add_xpath("opp_efg_pct", "td[12]/text()")
            loader.add_xpath("opp_fta_rate", "td[13]/text()")
            loader.add_xpath("opp_tov_pct", "td[14]/text()")
            loader.add_xpath("opp_oreb_pct", "td[15]/text()")
        ...
        if active_date > stop_date:
            ...
            url = f"https://www.nba.com/stats/teams/four-factors/?sort=TEAM_NAME&dir=-1&Season={self.season_dates['season_years']}&SeasonType=Regular%20Season&DateTo={month}%2F{day}%2F{year}"
...
if __name__ == "__main__":
    ...
    for year in [2022, 2021, 2020, 2019, 2018, 2017, 2016, 2015]:
        ...
        process.crawl(
            "NBA_four_factors_stats_spider", season_dates=season_info
        )
